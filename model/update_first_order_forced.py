...
class Func(eqx.Module):
    ...
    def __init__(self,channels=1,latent_size=2048,key=jax.random.PRNGKey(int(time.time()))):
        ...
        self.F = [
            eqx.nn.Linear(in_features=2*latent_size+2,out_features=latent_size,key=key),
        ...
        scale = 1.0
        w_f1 = jax.random.normal(key1,shape=(latent_size,2*latent_size+2)) * np.sqrt(2/(2*latent_size+2)) * scale
        w_f2 = jax.random.normal(key2,shape=(latent_size,latent_size)) * np.sqrt(2/(latent_size)) * scale
        w_g1 = jax.random.normal(key3,shape=(latent_size,2*latent_size+2)) * np.sqrt(2/(2*latent_size+2)) * scale
    ...
    def __call__(self,t,y,args):
        ...
        g = np.concatenate((y,t,forcing_signal))
        f = np.concatenate((y,t,forcing_signal))
