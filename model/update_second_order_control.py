...
class Func(eqx.Module):
    ...
    def __init__(self,channels=1,latent_size=2048,key=jax.random.PRNGKey(int(time.time()))):
        ...
        self.F_damp = [
            eqx.nn.Linear(in_features=latent_size,out_features=latent_size,key=key1),
            jax.nn.relu,
        ...
        self.F_osc = [
            eqx.nn.Linear(in_features=latent_size,out_features=latent_size,key=key3),
            jax.nn.relu,
            eqx.nn.Linear(in_features=latent_size,out_features=latent_size//2,key=key4),
            jax.nn.relu # maybe saturate this with different activation?
        ]
        self.G_damp = [
            eqx.nn.Linear(in_features=latent_size,out_features=latent_size,key=key5),
            jax.nn.relu,
        ...
        self.G_tot = [
            eqx.nn.Linear(in_features=latent_size,out_features=latent_size,key=key7),
            jax.nn.relu,
        ...
        scale = 1.0
        w_fd1 = jax.random.normal(key1,shape=(latent_size,latent_size )) * np.sqrt(2/(latent_size)) * scale
        w_fd2 = jax.random.normal(key2,shape=(latent_size//2,latent_size)) * np.sqrt(2/(latent_size)) * scale
        w_fo1 = jax.random.normal(key3,shape=(latent_size,latent_size )) * np.sqrt(2/(latent_size)) * scale
        w_fo2 = jax.random.normal(key4,shape=(latent_size//2,latent_size)) * np.sqrt(2/(latent_size)) * scale
        
        
        w_gd1 = jax.random.normal(key5,shape=(latent_size,latent_size)) * np.sqrt(2/(latent_size)) * scale
        w_gd2 = jax.random.normal(key6,shape=(latent_size//2,latent_size)) * np.sqrt(2/(latent_size)) * scale
        w_gt1 = jax.random.normal(key7,shape=(latent_size,latent_size)) * np.sqrt(2/(latent_size)) * scale
